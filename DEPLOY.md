...
**Админ-панель** (страница `/admin`): в настройках проекта добавь переменные (по желанию; если не заданы, используются логин `tv1p` и пароль по умолчанию):
- **ADMIN_LOGIN** — логин (по умолчанию `tv1p`)
- **ADMIN_PASSWORD** — пароль входа
- **ADMIN_TOKEN** — секрет для Bearer-запросов (если не задан, используется `ADMIN_PASSWORD`)
